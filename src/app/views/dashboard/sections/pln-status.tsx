...
import { PLNData } from "@/types/telemetry";
...
export default function PLNStatus() {
  // 2. Panggil hook untuk data realtime
  const { pln, loading } = useRealtimeData();

  // 3. Helper untuk menangani data null saat loading
  const plnData: PLNData = pln || {
    voltage: 0,
    current: 0,
    power: 0,
    energy: 0,
  ...
  const isPLNActive = plnData.status;
  ...
  const gradient = isPLNActive
    ? "from-blue-400 to-blue-500"
    : "from-gray-400 to-gray-500";
  const borderColor = isPLNActive ? "border-blue-600" : "border-gray-600";
  const textColor = isPLNActive ? "text-blue-600" : "text-gray-600";
  const Icon = isPLNActive ? CheckCircle : XCircle;
            ...
            <div className="grid grid-cols-2 gap-2 text-xs sm:text-sm">
              <div>
                ...
                <p className="font-semibold text-slate-700">
                  {isPLNActive ? formatVoltage(plnData.voltage) : "---"}
                </p>
              </div>
              <div>
                <p className="text-slate-500">Current</p>
                <p className="font-semibold text-slate-700">
                  {isPLNActive ? formatCurrent(plnData.current) : "---"}
                </p>
              </div>
              <div>
                <p className="text-slate-500">Power</p>
                <p className="font-semibold text-slate-700">
                  {isPLNActive ? formatPower(plnData.power) : "---"}
                </p>
              </div>
              <div>
                <p className="text-slate-500">Frequency</p>
                <p className="font-semibold text-slate-700">
                  {isPLNActive ? formatFrequency(plnData.frequency) : "---"}
                </p>
              </div>
               <div>
                <p className="text-slate-500">Power Factor</p>
                <p className="font-semibold text-slate-700">
                  {isPLNActive ? formatPowerFactor(plnData.pf) : "---"}
                </p>
              </div>
            </div>
          </div>
